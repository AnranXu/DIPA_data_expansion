    ...
    def validation_step (self, val_batch, batch_idx):
        ...
        for i, (output_name, output_dim) in enumerate(self.output_channel.items()):
            ...
            if output_name == 'informativeness':
                ...
                accuracy(torch.round(y_preds[i] * 6).to('cuda'), (y[:,i] * 6).type(torch.LongTensor).to('cuda'))
                precision(torch.round(y_preds[i] * 6).to('cuda'), (y[:,i] * 6).type(torch.LongTensor).to('cuda'))
                recall(torch.round(y_preds[i] * 6).to('cuda'), (y[:,i] * 6).type(torch.LongTensor).to('cuda'))
                f1score(torch.round(y_preds[i] * 6).to('cuda'), (y[:,i] * 6).type(torch.LongTensor).to('cuda'))
            else:
